...
> **Примечание:** Этот документ описывал **Pipeline A** (WSD + Synset + SmartCardGenerator), который **удалён** из репозитория. История и причины — в `docs/HISTORY_PIPELINE_A.md`. Текущая генерация карточек — только **Pipeline B**: см. `docs/PIPELINES_OVERVIEW.md` и `scripts/run_pipeline_b_batch.py` (create / download).

---

## Текущий статус (Pipeline A — удалён)
