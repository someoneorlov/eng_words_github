...
- финальным вердиктом **PASS / FAIL** (см. § 6).

---

## 1) Входные данные (актуальные пути)

### 1.1. Must-have (должны существовать после прогона Pipeline B)
...
### 1.2. Optional (может отсутствовать; если отсутствует — честно отметь в отчёте)
...
### 1.3. Stage 1 (полные данные книги): что реально есть сейчас
...
## 1.5. Автоматические отчёты (сгенерируй перед ручным QC)
...
### 2.2. Как выбирать внутри группы
...
## 3) Чеклист качества для каждой карточки (ручной QA)

По каждой карточке выставь: **OK / Minor / Major / Invalid**.

### 3.1. Валидность структуры (Schema)
...
> **Примечание:** если ваш pipeline допускает «0 примеров» как временный деградированный режим — отметь это отдельно, но **в PASS/FAIL** считай как провал качества (см. § 6).

### 3.2. Соответствие «lemma ↔ examples»
...
### 3.6. Смысловая кластеризация (несколько карточек на лемму)
...
### 3.7. Качество примеров для Anki
...
## 4) Метрики (на 30–50 карточках)

Считай **по карточкам** и при необходимости **по леммам**.

### 4.1. Основные метрики качества
...
## 5) Структура QC-отчёта (что именно написать)
...
### 5.3. Ошибки по категориям (таблица)

E1…E8: `count`, `%`.

### 5.4. Примеры проблем (10–15 штук)
...
## 7) Что сравнивать между прогонами (если есть baseline)
...
## 8) Быстрый протокол на одну карточку (~30 сек)
...
- Автоматический quality gate (sample runs): `docs/PIPELINE_B_QUALITY_GATE.md`
