...
def filter_content_words(tokens: pd.DataFrame) -> pd.DataFrame:
    ...
    logger.info(f"Content words: {len(content):,} tokens, {content['lemma'].nunique():,} unique lemmas")
    return content
...
def reconstruct_sentences(tokens: pd.DataFrame) -> pd.DataFrame:
    ...
    sentences = tokens.groupby('sentence_id').apply(
        lambda g: ' '.join(g.sort_values('position')['surface'].astype(str))
