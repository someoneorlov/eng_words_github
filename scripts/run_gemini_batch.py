...
def build_extraction_prompt(lemma: str, examples: list[tuple[int, str]]) -> str:
    ...
    numbered_examples = "\n".join(
        f"{i+1}. (id={sent_id}) {text}" for i, (sent_id, text) in enumerate(examples)
    )
...
def estimate():
    ...
    standard_cost = (
        total_input * standard_input_price + total_output * standard_output_price
    ) / 1_000_000
    ...
    print()
    print("Cost comparison (gemini-3-flash-preview):")
    print(f"  Standard API: ${standard_cost:.2f}")
    print(f"  Batch API:    ${batch_cost:.2f}")
    print(
        f"  Savings:      ${standard_cost - batch_cost:.2f} ({(1 - batch_cost/standard_cost)*100:.0f}%)"
    )
