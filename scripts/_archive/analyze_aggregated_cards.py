...
def analyze_aggregated_cards():
    ...
    for col in df.columns:
        ...
        if isinstance(value, (list, dict)):
            print(
                f"  {col}: {type(value).__name__} (len={len(value) if hasattr(value, '__len__') else 'N/A'})"
            )
    ...
    if "synset_group" in df.columns:
        ...
        if group_sizes:
            ...
            print(
                f"  Группы с 1 synset: {sum(1 for s in group_sizes if s == 1)} ({sum(1 for s in group_sizes if s == 1)/len(group_sizes)*100:.1f}%)"
            )
            print(
                f"  Группы с 2+ synsets: {sum(1 for s in group_sizes if s > 1)} ({sum(1 for s in group_sizes if s > 1)/len(group_sizes)*100:.1f}%)"
            )
    ...
    if "sentence_ids" in df.columns:
        ...
        if sentence_counts:
            print(
                f"  Среднее количество примеров: {sum(sentence_counts) / len(sentence_counts):.2f}"
            )
            print(f"  Минимум: {min(sentence_counts)}")
            print(f"  Максимум: {max(sentence_counts)}")
            print(
                f"  Карточки без примеров: {sum(1 for c in sentence_counts if c == 0)} ({sum(1 for c in sentence_counts if c == 0)/len(sentence_counts)*100:.1f}%)"
            )
