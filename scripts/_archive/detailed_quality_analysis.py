...
def check_alignment(card: dict) -> dict[str, bool]:
    ...
    translation_matches = bool(translation) and bool(
        re.search(r"[а-яё]", translation, re.IGNORECASE)
    )
...
def print_detailed_card(card: dict, analysis: CardAnalysis, index: int, total: int):
    ...
    print(
        f"  Определение: {'✅' if analysis.has_definition else '❌'} ({analysis.definition_length} символов)"
    )
    print(
        f"  Перевод:     {'✅' if analysis.has_translation else '❌'} ({analysis.translation_length} символов)"
    )
    print()
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Детальная автоматизированная проверка качества карточек"
    )
    parser.add_argument(
        "--input",
        type=Path,
    ...
    print(
        f"  Все компоненты:  {summary['components_presence']['all_components']} ({summary['components_presence']['all_components_pct']:.1f}%)"
    )
    print(
        f"  С примерами:     {summary['components_presence']['has_examples']} ({summary['components_presence']['has_examples_pct']:.1f}%)"
    )
    print(
        f"  С определением:  {summary['components_presence']['has_definition']} ({summary['components_presence']['has_definition_pct']:.1f}%)"
    )
    print(
        f"  С переводом:     {summary['components_presence']['has_translation']} ({summary['components_presence']['has_translation_pct']:.1f}%)"
    )
    print()

    print("✅ КАЧЕСТВО:")
    print(
        f"  Валидные примеры:        {summary['quality_metrics']['valid_examples']} ({summary['quality_metrics']['valid_examples_pct']:.1f}%)"
    )
    print(
        f"  Согласованные:           {summary['quality_metrics']['aligned_components']} ({summary['quality_metrics']['aligned_components_pct']:.1f}%)"
    )
    print(
        f"  Проблемные (≤2):         {summary['quality_metrics']['problematic_cards']} ({summary['quality_metrics']['problematic_cards_pct']:.1f}%)"
    )
    print(
        f"  С критическими ошибками: {summary['quality_metrics']['critical_issues']} ({summary['quality_metrics']['critical_issues_pct']:.1f}%)"
    )
    print()
    ...
    print(
        f"  Много примеров (≥5): {summary['examples_statistics']['cards_with_many']} ({summary['examples_statistics']['cards_with_many_pct']:.1f}%)"
    )
    print(
        f"  Мало примеров (<3):  {summary['examples_statistics']['cards_with_few']} ({summary['examples_statistics']['cards_with_few_pct']:.1f}%)"
    )
    print()
    ...
    if problematic:
        ...
        for a in problematic:
            print(
                f"  {a.lemma} ({a.pos}): {a.overall_quality_score}/5 - {', '.join(a.critical_issues[:2])}"
            )
        print()
