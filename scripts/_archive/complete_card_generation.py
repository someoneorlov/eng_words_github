...
def analyze_cards(cards: list[SmartCard]) -> dict[str, Any]:
    ...
    logger.info(
        f"  Invalid examples:    {stats['invalid'] - stats['empty']} ({(stats['invalid'] - stats['empty'])/total*100:.1f}%)"
    )
    ...
    if invalid_examples_only:
        ...
        for i, (card, validation) in enumerate(invalid[:5], 1):
            if validation is not None:
                logger.info(
                    f"    {i}. {card.lemma} ({card.pos}) - {len(validation.invalid_examples)} invalid examples"
                )
                logger.info(
                    f"       Invalid: {validation.invalid_examples[0][:80]}..."
...
def main():
    ...
    logger.info("\n## Verification")
    all_valid = all(validate_card_examples(c).is_valid for c in final_cards if c.selected_examples)
    ...
    if not all_valid:
        invalid_final = [
            c for c in final_cards if c.selected_examples and not validate_card_examples(c).is_valid
        ]
        logger.warning(f"  ⚠️  {len(invalid_final)} cards still have invalid examples:")
        for card in invalid_final[:10]:
            validation = validate_card_examples(card)
            logger.warning(
                f"      - {card.lemma} ({card.pos}): {len(validation.invalid_examples)} invalid"
            )
    ...
    for card_dict in final_data:
        for key, value in card_dict.items():
            if hasattr(value, "tolist"):  # numpy array
                ...
                card_dict[key] = [v.tolist() if hasattr(v, "tolist") else v for v in value]
    ...
    logger.info("")
    logger.info(
        f"  Empty → Fixed:     {initial_stats['empty']} → {final_stats['empty']} ({initial_stats['empty'] - final_stats['empty']} fixed)"
    )
    logger.info(
        f"  Invalid → Fixed:   {initial_stats['invalid']} → {final_stats['invalid']} ({initial_stats['invalid'] - final_stats['invalid']} fixed)"
    )
