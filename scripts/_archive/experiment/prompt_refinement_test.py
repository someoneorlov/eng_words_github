...
def run_baseline(test_data: dict, model: str = "gemini-2.5-flash"):
    ...
    for lemma, data in test_data.items():
        ...
        try:
            ...
            logger.info(
                f"  {status} {lemma}: {len(result.cards)} cards (expected {data['expected_cards']})"
            )
...
def evaluate_results(results: dict):
    ...
    oversplit_reduction = (
        (oversplit_before - oversplit_after) / oversplit_before * 100 if oversplit_before > 0 else 0
    )
    ...
    print(
        f"CORRECT preservation: {correct_before} → {correct_after} ({correct_preservation:.1f}% preserved)"
    )

    success = oversplit_reduction >= 30 and correct_preservation >= 90
    print(f"\n{'✅ SUCCESS' if success else '❌ NEEDS IMPROVEMENT'}")
    print(
        f"  - Oversplit reduction ≥ 30%: {oversplit_reduction:.1f}% {'✅' if oversplit_reduction >= 30 else '❌'}"
    )
    print(
        f"  - Correct preservation ≥ 90%: {correct_preservation:.1f}% {'✅' if correct_preservation >= 90 else '❌'}"
    )
