...
def check_status():
    ...
    if OUTPUT_FILE.exists():
            ...
            if len(data) > 0:
                ...
                if has_card_full:
                    ...
                    if len(data) == EXPECTED_COUNT:
                        ...
                        print(f"   Последняя карточка: {last.get('lemma', 'N/A')} ({last.get('pos', 'N/A')})")
                    else:
                        last = data[-1]
                        print(f"\n⏳ Регенерация в процессе...")
                        print(f"   Последняя обработанная: {last.get('lemma', 'N/A')} ({last.get('pos', 'N/A')})")
        ...
        print(f"   Процесс может еще не начать запись в файл")
