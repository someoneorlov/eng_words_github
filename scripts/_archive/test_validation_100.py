...
def test_validation_100():
    ...
    results = []
    stats_by_pos = defaultdict(
        lambda: {"total": 0, "has_valid": 0, "valid_examples": 0, "invalid_examples": 0}
    )
    ...
    logger.info(
        f"    Без валидных примеров: {total - has_valid} ({(total-has_valid)/total*100:.1f}%)"
    )
    logger.info(f"    Всего примеров проверено: {total_examples}")
    logger.info(
        f"    Валидных примеров: {total_valid} ({total_valid/total_examples*100:.1f}% от всех)"
    )
    logger.info(
        f"    Невалидных примеров: {total_invalid} ({total_invalid/total_examples*100:.1f}% от всех)"
    )
    ...
    for pos in sorted(stats_by_pos.keys()):
        ...
        if stats["total"] > 0:
            ...
            valid_pct = (
                ...
                / (stats["valid_examples"] + stats["invalid_examples"])
                * 100
                if (stats["valid_examples"] + stats["invalid_examples"]) > 0
                else 0
            )
            logger.info(f"    {pos}:")
            ...
            logger.info(
                f"      Валидных примеров: {stats['valid_examples']} ({valid_pct:.1f}% от всех)"
            )
    ...
    # Метрики (объяснение)
    logger.info("\n  Метрики валидации:")
    logger.info("    Precision (точность):")
    logger.info("      = True Positives / (True Positives + False Positives)")
    logger.info("      = Доля правильно определенных валидных примеров")
    logger.info("      ⚠️  Требует ручной проверки или ground truth для расчета")
    logger.info("    Recall (полнота):")
    logger.info("      = True Positives / (True Positives + False Negatives)")
    logger.info("      = Доля найденных валидных примеров от всех валидных")
    logger.info("      ⚠️  Требует ручной проверки или ground truth для расчета")
    logger.info("    Статистика валидации:")
    logger.info(f"      Валидных найдено: {total_valid} ({total_valid/total_examples*100:.1f}%)")
    logger.info(
        f"      Невалидных отфильтровано: {total_invalid} ({total_invalid/total_examples*100:.1f}%)"
    )
    ...
    if no_valid:
        ...
        for i, r in enumerate(no_valid[:10], 1):
            logger.info(
                f"    {i}. {r['lemma']} ({r['pos']}) - {r['primary_synset']} - примеров: {r['total_examples']}"
            )
