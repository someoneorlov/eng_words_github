...
from eng_words.wsd_gold.models import GoldExample, LLMUsage
...
def download():
    ...
    safe_id_map = {
        ex.example_id.replace(":", "_").replace("|", "_"): ex.example_id for ex in examples
    }
...
def wait():
    ...
    while True:
        ...
        if hasattr(batch, "request_counts"):
            counts = batch.request_counts
            print(
                f"Status: {status} | Succeeded: {counts.succeeded} | Processing: {counts.processing}"
            )
        else:
