...
def identify_problematic_cards(cards: list[dict], n: int = 50) -> list[dict]:
    ...
    few_examples = [
        c for c in cards
        if c.get("selected_examples") and len(c["selected_examples"]) <= 2
    ]
...
def find_matching_new_cards(
    problematic_old: list[dict], new_cards: list[dict]
) -> dict[str, dict]:
...
def compare_card_quality(old_card: dict, new_card: dict) -> dict[str, Any]:
    ...
    comparison = {
        ...
        "match_type": "exact" if old_card.get("primary_synset") == new_card.get("primary_synset") else "different",
    }
...
def main():
    ...
    print(f"  Без примеров:     {summary['old_approach']['cards_without_examples']} ({summary['old_approach']['cards_without_examples_pct']:.1f}%)")
    print(f"  Валидные:         {summary['old_approach']['valid_cards']} ({summary['old_approach']['valid_cards_pct']:.1f}%)")
    print()
    print("Новый подход (synset):")
    print(f"  Без примеров:     {summary['new_approach']['cards_without_examples']} ({summary['new_approach']['cards_without_examples_pct']:.1f}%)")
    print(f"  Валидные:         {summary['new_approach']['valid_cards']} ({summary['new_approach']['valid_cards_pct']:.1f}%)")
    print()
    print("Улучшения:")
    print(f"  Примеров больше:  {summary['improvements']['examples_improved']} ({summary['improvements']['examples_improved_pct']:.1f}%)")
    ...
    for i, comp in enumerate(top_improvements, 1):
        print(f"  {i}. {comp['lemma']}: {comp['old_examples_count']} → {comp['new_examples_count']} примеров")
