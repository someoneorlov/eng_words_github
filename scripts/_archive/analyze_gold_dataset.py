...
def analyze_gold_dataset():
            ...
            if isinstance(value, dict):
                ...
                for subkey, subvalue in value.items():
                    if isinstance(subvalue, list) and len(subvalue) > 0:
                        print(f"    {subkey}: list[{len(subvalue)}] (первый элемент: {subvalue[0] if isinstance(subvalue[0], str) else type(subvalue[0]).__name__})")
                    else:
                        print(f"    {subkey}: {subvalue}")
            elif isinstance(value, list):
                print(f"  {key}: list[{len(value)}]")
                if len(value) > 0:
                    print(f"    Первый элемент: {value[0] if isinstance(value[0], str) else value[0]}")
            else:
                print(f"  {key}: {value}")
    ...
    print(f"  Распределение по POS:")
    for pos, count in sorted(pos_counts.items(), key=lambda x: -x[1]):
        print(f"    {pos}: {count} ({count/len(dev_examples)*100:.1f}%)")
    
    # Примеры для валидации
    print(f"\n🔍 Примеры для использования в валидации:")
    print(f"  Каждый пример содержит:")
    print(f"    - context_window: предложение с целевым словом")
    print(f"    - target.lemma: лемма слова")
    print(f"    - target.pos: часть речи")
    print(f"    - gold_synset_id: правильный synset (эталон)")
    print(f"    - candidates: список возможных synsets")
    ...
    if test_path.exists():
        ...
        print(f"  ⚠️  ВАЖНО: Использовать только для финального сравнения!")
        print(f"  Не смотреть во время разработки!")
    else:
    ...
    print(f"  2. Test locked - только для финального сравнения")
    print(f"  3. Для валидации: проверять, правильно ли определяется соответствие")
    print(f"     примера synset_group (precision/recall)")
