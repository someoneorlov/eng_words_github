...
def analyze_card_quality(card) -> dict:
    ...
    stats["selected_examples_too_long"] = sum(1 for l in stats["selected_examples_lengths"] if l > 50)
    stats["generated_examples_too_long"] = sum(1 for l in stats["generated_examples_lengths"] if l > 50)
...
def test_new_prompt_150(limit: int = 150):
    ...
    if card_stats:
        ...
        logger.info(f"      Всего примеров (мин/макс): {np.min(total_examples)} / {np.max(total_examples)}")
        ...
        logger.info(f"      Карточек с 3-5 примерами: {cards_with_3_5_examples} ({cards_with_3_5_examples/len(card_stats)*100:.1f}%)")
        ...
        if all_selected_lengths:
            ...
            logger.info(f"      Мин/макс: {np.min(all_selected_lengths)} / {np.max(all_selected_lengths)} слов")
            logger.info(f"      Идеальная длина (10-30): {sum(1 for l in all_selected_lengths if 10 <= l <= 30)} / {len(all_selected_lengths)} ({sum(1 for l in all_selected_lengths if 10 <= l <= 30)/len(all_selected_lengths)*100:.1f}%)")
            logger.info(f"      Слишком длинные (>50): {sum(1 for l in all_selected_lengths if l > 50)} / {len(all_selected_lengths)} ({sum(1 for l in all_selected_lengths if l > 50)/len(all_selected_lengths)*100:.1f}%)")
        
        if all_generated_lengths:
            logger.info(f"\n    Длина сгенерированных примеров:")
            logger.info(f"      Средняя: {np.mean(all_generated_lengths):.1f} слов")
            logger.info(f"      Мин/макс: {np.min(all_generated_lengths)} / {np.max(all_generated_lengths)} слов")
            logger.info(f"      Идеальная длина (10-30): {sum(1 for l in all_generated_lengths if 10 <= l <= 30)} / {len(all_generated_lengths)} ({sum(1 for l in all_generated_lengths if 10 <= l <= 30)/len(all_generated_lengths)*100:.1f}%)")
            logger.info(f"      Слишком длинные (>50): {sum(1 for l in all_generated_lengths if l > 50)} / {len(all_generated_lengths)} ({sum(1 for l in all_generated_lengths if l > 50)/len(all_generated_lengths)*100:.1f}%)")
        
        # Определения
        definition_lengths = [s["definition_length"] for s in card_stats]
        logger.info(f"\n    Определения:")
        logger.info(f"      Средняя длина: {np.mean(definition_lengths):.1f} слов")
        logger.info(f"      Мин/макс: {np.min(definition_lengths)} / {np.max(definition_lengths)} слов")
        logger.info(f"      Соответствуют требованию (≤15 слов): {sum(1 for l in definition_lengths if l <= 15)} / {len(definition_lengths)} ({sum(1 for l in definition_lengths if l <= 15)/len(definition_lengths)*100:.1f}%)")
        
        # Переводы
        cards_with_translation = sum(1 for s in card_stats if s["has_translation"])
        logger.info(f"\n    Переводы:")
        logger.info(f"      Карточек с переводом: {cards_with_translation} / {len(card_stats)} ({cards_with_translation/len(card_stats)*100:.1f}%)")
        
        # Quality scores
        cards_with_scores = sum(1 for s in card_stats if s["quality_scores_count"] > 0)
        logger.info(f"\n    Quality scores:")
        logger.info(f"      Карточек с оценками качества: {cards_with_scores} / {len(card_stats)} ({cards_with_scores/len(card_stats)*100:.1f}%)")
